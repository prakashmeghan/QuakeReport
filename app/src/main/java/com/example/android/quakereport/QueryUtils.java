...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray jsonArray = jsonObject.getJSONArray(ConstantUtil.NODE_FEATURES);
            for (int i=0; i < jsonArray.length(); i++){
                JSONObject obj = jsonArray.getJSONObject(i);
                JSONObject objProp = obj.getJSONObject(ConstantUtil.NODE_PROPERTIES);
                double mag = objProp.getDouble(ConstantUtil.NODE_MAG);
                String place = objProp.getString(ConstantUtil.NODE_PLACE);
                long quakeTime = objProp.getLong(ConstantUtil.NODE_TIME);
                String quakeUrl = objProp.getString(ConstantUtil.NODE_URL);

                Earthquake earthquake = new Earthquake();
                earthquake.setMag(mag);
                earthquake.setPlace(place);
                earthquake.setQuakeTime(quakeTime);
                earthquake.setQuakeUrl(quakeUrl);
